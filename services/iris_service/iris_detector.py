...
class IrisDetector:
    ...
    def extract_iris_template(self, image: np.ndarray) -> dict:
        ...
        # Calculate EAR (Eye Aspect Ratio) to check if eyes are open
        left_ear = self._calculate_ear(landmarks, w, h, "left")
        right_ear = self._calculate_ear(landmarks, w, h, "right")

        if left_ear < 0.20 and right_ear < 0.20:
            return {"success": False, "message": "Eye region not clear for iris verification"}
        ...
        # Quality check: Dark glasses / extremely poor quality
        if q_score_l < 0.3 and q_score_r < 0.3:
            return {"success": False, "message": "Eye region not clear for iris verification"}
        ...
        return {
            ...
            "left_eye_score": q_score_l,
            "right_eye_score": q_score_r
        }
    ...
    def _extract_features(self, eye_image: np.ndarray) -> Tuple[np.ndarray, float]:
        ...
        if brightness < 40 or brightness > 220:
            quality *= 0.2  # Heavy penalty for dark glasses or washed out
            
        return hist, float(quality)

    def _calculate_ear(self, landmarks, w, h, eye="left"):
        """Calculates Eye Aspect Ratio to detect closed eyes."""
        def pt(idx):
            return np.array([landmarks[idx].x * w, landmarks[idx].y * h])
            
        if eye == "left":
            p2_p6 = np.linalg.norm(pt(159) - pt(145))
            p3_p5 = np.linalg.norm(pt(158) - pt(153))
            p1_p4 = np.linalg.norm(pt(33) - pt(133))
        else:
            p2_p6 = np.linalg.norm(pt(386) - pt(374))
            p3_p5 = np.linalg.norm(pt(385) - pt(380))
            p1_p4 = np.linalg.norm(pt(362) - pt(263))
