...
class EnrollmentManager:
    ...
    async def enroll(self, employee_id: str, name: str, role: str = None, 
        ...
        if face_images:
            for i, img_b64 in enumerate(face_images):
                face_data = await self.face_client.detect_and_embed(img_b64)
        ...
        if eye_images:
            for i, img_b64 in enumerate(eye_images):
                iris_data = await self.iris_client.extract_template(img_b64)
